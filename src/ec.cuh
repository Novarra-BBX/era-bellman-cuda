...
template <typename FD, unsigned B_ABS, bool B_NEG = false> struct ec {
  typedef FD field;
  ...
  struct point_affine {
    ...
    static __host__ __device__ __forceinline__ bool is_on_curve(const point_affine &point, const FD &fd) {
      ...
      const storage a = y2;
      const storage b = (B_NEG ? fd.sub(x3, fd.mul(B_ABS, fd.get_one())) : fd.add(x3, fd.mul(B_ABS, fd.get_one())));
      return fd.eq(a, b);
    }
  };
  ...
  struct point_projective {
    ...
    static __host__ __device__ __forceinline__ bool is_on_curve(const point_projective &point, const FD &fd) {
      ...
      const storage b = (B_NEG ? fd.sub(x3, fd.mul(B_ABS, z3)) : fd.add(x3, fd.mul(B_ABS, z3)));
      return fd.eq(a, b);
    }
  };
  ...
  struct point_jacobian {
    ...
    static bool __host__ __device__ __forceinline__ is_on_curve(const point_jacobian &point, const FD &fd) {
      ...
      const storage a = y2;
      const storage b = (B_NEG ? fd.sub(x3, fd.mul(B_ABS, z6)) : fd.add(x3, fd.mul(B_ABS, z6)));
      return fd.eq(a, b);
    }
  };
  ...
  static __host__ __device__ __forceinline__ point_projective add(const point_projective &p1, const point_projective &p2, const FD &fd) {
    ...
    const storage t20 = (B_NEG ? fd.sub(fd.get_zero(), fd.template mul<2>(3 * B_ABS, t02)) : fd.template mul<2>(3 * B_ABS, t02));
    const storage t21 = fd.template add<2>(t01, t20);                        // t21 ← t01 + t20   < 2
    const storage t22 = fd.template sub<2>(t01, t20);                        // t22 ← t01 − t20   < 2
    const storage t23 = (B_NEG ? fd.sub(fd.get_zero(), fd.template mul<2>(3 * B_ABS, t17)) : fd.template mul<2>(3 * B_ABS, t17)); // t23 <- b3 · t17
#ifdef __CUDA_ARCH__
    // X3 ← t07 · t22 - t12 · t23
  ...
  static __host__ __device__ __forceinline__ point_projective add(const point_projective &p1, const point_affine &p2, const FD &fd) {
    ...
    storage t2 = (B_NEG ? fd.sub(fd.get_zero(), fd.template mul<2>(3 * B_ABS, Z1)) : fd.template mul<2>(3 * B_ABS, Z1));
    storage Z3 = fd.template add<2>(t1, t2);          // 15. Z3 ← t1 + t2
    t1 = fd.template sub<2>(t1, t2);                  // 16. t1 ← t1 − t2
    Y3 = (B_NEG ? fd.sub(fd.get_zero(), fd.template mul<2>(3 * B_ABS, Y3)) : fd.template mul<2>(3 * B_ABS, Y3)); // Y3 <- b3 · Y3
  ...
  static __host__ __device__ __forceinline__ point_projective dbl(const point_projective &point, const FD &fd) {
    ...
    t2 = (B_NEG ? fd.sub(fd.get_zero(), fd.template mul<2>(3 * B_ABS, t2)) : fd.template mul<2>(3 * B_ABS, t2));
