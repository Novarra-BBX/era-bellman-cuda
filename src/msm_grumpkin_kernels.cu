...
__host__ cudaError_t compute_bucket_indexes(const fd_p::storage *scalars, const unsigned windows_count, const unsigned window_bits, unsigned *bucket_indexes,
...
__launch_bounds__(MAX_THREADS)
  ...
  buckets[bucket_index] = point_xyzz::point_at_infinity(fd_q());
}
