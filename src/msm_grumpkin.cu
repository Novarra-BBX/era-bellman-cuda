...
unsigned get_windows_count(const unsigned window_bits_count) { return (fd_p::MBC - 1) / window_bits_count + 1; }
...
cudaError_t schedule_execution(const extended_configuration &cfg, const bool dry_run) {
  ...
  typedef allocation<fd_p::storage> scalars;
  ...
  const unsigned top_window_unused_bits = windows_count_pass_one * bits_count_pass_one - fd_p::MBC;
  ...
  for (unsigned inputs_offset = 0, log_inputs_count = log_min_inputs_count; inputs_offset < scalars_count;) {
    ...
    if (!dry_run) {
      ...
      if (copy_scalars) {
        ...
        const size_t inputs_size = sizeof(fd_p::storage) << log_inputs_count;
  ...
  for (unsigned i = 0;; i++) {
    ...
    if (target_bits_count == 1) {
      ...
      const unsigned result_windows_count = fd_p::MBC;
      if (copy_results)
